...
void printDHCPopts(uchar *ppkt)
{
	...
	while (*ppkt != DHCP_OPTIONS_END)
	{
		...
		case DHCP_OPTIONS_DOMAIN_NAME:
			printf("Domain name \"");
			for (i = 0; i < len; i++) { printf("%c", *ppkt++); }
			printf("\"");
			break;
		...
		case DHCP_OPTIONS_LEASETIME:
			i = 0;
			for ( ; len > 0; len--) { i = i * 256 + *ppkt++; }
			printf("Lease time %u", i);
			break;
